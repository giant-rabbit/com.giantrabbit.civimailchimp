...
class CRM_CiviMailchimp_BAO_Group extends CRM_CiviMailchimp_DAO_Group {
   ...
   */
  static function getMailchimpSyncSettingsByGroupId($group_id) {
  ...
  static function updateSettings($params) {
    ...
    try {
      $existing_group = self::getMailchimpSyncSettingsByGroupId($params['civicrm_group_id']);
      if ($existing_group) {
  ...
  static function deleteSettings($params) {
    $transaction = new CRM_Core_Transaction();
    try {
      $civimailchimp_group = self::getMailchimpSyncSettingsByGroupId($params['civicrm_group_id']);
